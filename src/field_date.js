...
Blockly.FieldDate = function (
   ...
   * @returns {String}
   */
  Blockly.FieldDate.prototype.getDateString = function () {
    if (!this._DATE || this._DATE.invalid || !this._TOKEN) {
      return null
    }
    return this._DATE.toFormat(this._TOKEN)
  };
   ...
   */
   Blockly.FieldDate.prototype.getDate = function () {
    return new goog.date.DateTime.fromTimestamp(
      lib.DateTime.fromFormat(
        this.getValue(),
        this._TOKEN).ts
      )
  };
   ...
   * @protected
   */
  Blockly.FieldDate.prototype.DEFAULT_VALUE = this.getDateString()

  Blockly.FieldDate.superClass_.constructor.call(this, value, validator);
...
Blockly.FieldDate.prototype.doClassValidation_ = function (newValue = undefined) {
  ...
  const date = lib.DateTime.fromFormat(newValue, this._TOKEN);
  if (date.invalid) {
    return null;
  }
  return newValue;
...
Blockly.FieldDate.prototype.render_ = function () {
  ...
  if (this.picker_ && this.isTextValid_) {
    this.picker_.setDate(this.getDate());
    this.updateEditor_();
  }
};
...
Blockly.FieldDate.prototype.applyColour = function () {
  ...
  this.selectedColour_ = this.sourceBlock_.style.colourPrimary;
  this.updateEditor_();
};
...
Blockly.FieldDate.prototype.updateEditor_ = function () {
  ...
  if (this.oldTodayElement_) {
    ...
    this.oldTodayElement_.style.fontWeight = null;
  }
  ...
  if (this.oldSelectedElement_ && this.oldSelectedElement_ != today) {
    ...
    this.oldSelectedElement_.style.fontWeight = null;
  }
...
Blockly.FieldDate.prototype.onDateSelected_ = function (event) {
  ...
  if (event.date) {
    this._DATE = lib.DateTime.fromISO(event.date.toIsoString())
  }
